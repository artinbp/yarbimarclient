<script setup>
import { computed, onMounted } from 'vue';
// import { deleteIcon } from '@/assets/icon';
import store from '@/store';
import {currency} from '@/assets/utils';
// import VueIcon from '@/components/output/vueIcon';

const cart = computed(()=>store.getters.getCart)



const cartProducts = computed(()=>store.getters.getCartProduct)
const total = computed(()=> store.getters.getCartTotal)

// const deleteAddress = async (e) => {
//   await store.dispatch('deleteAddress', e).then(() => {
//     store.dispatch('generateAddress')
//   })
// }
const addCart = async (e) =>{
  await store.dispatch('appendCart', { product_id:e }).then(async ()=>await store.dispatch('generateCart'))
}
const removeCart = async (e) =>{
  await store.dispatch('removeCart', { product_id:e }).then(async ()=>await store.dispatch('generateCart'))
}

// const wishList = computed(() => store.getters.getWishList)
onMounted( async () => {
   await store.dispatch('generateCart')
   await cart.value.items.forEach((item)=>{
     store.dispatch('generateCartProducts', { data: item.product_id, count:item.quantity })
  })
})

</script>
<template>
  <!-- breadcrum -->
  <div class="flex flex-row-reverse items-center justify-between">
    <div class="py-4 container rtl flex gap-3 items-center">
      <router-link to="/" class="text-primary text-base">
        <i class="fas fa-home"></i>
      </router-link>
      <span class="text-sm text-gray-400"><i class="fas fa-chevron-right rotate-180"></i></span>
      <p class="text-gray-600 font-semibold uppercase">حساب من</p>
    </div>
  </div>
  <!-- breadcrum end -->

  <!-- account wrapper -->

    <!-- sidebar -->
    <div class="container rtl lg:grid grid-cols-12 gap-6 items-start pb-16 pt-4">
      <!-- product cart -->
      <div class="xl:col-span-9 lg:col-span-8">
        <!-- cart title -->
        <div class="bg-gray-200 py-2 pr-12 pl-20 xl:pl-28 mb-4 hidden md:flex">
          <p class="text-gray-600 text-center">Product</p>
          <p class="text-gray-600 text-center mr-auto ml-16 xl:ml-24">Quantity</p>
          <p class="text-gray-600 text-center">Total</p>
        </div>
        <!-- cart title end -->

        <!-- shipping carts -->
        <div class="space-y-4">
          <!-- single cart -->
          <div v-for="(pro,i) in cart.items" :key="i"  class="flex items-center md:justify-between gap-4 md:gap-6 p-4 border border-gray-200 rounded flex-wrap md:flex-nowrap">
            <!-- cart image -->
            <div class="w-32 flex-shrink-0">
              <img :src=" cartProducts[i]?.thumbnail_url " class="w-full h-[94px] object-center object-cover">
            </div>
            <!-- cart image end -->
            <!-- cart content -->
            <div class="md:w-1/3 w-full">
              <h2 class="text-gray-800 mb-3 xl:text-xl textl-lg font-medium uppercase">
                {{ cartProducts[i]?.title }}
              </h2>
              <p class="text-primary font-semibold"> {{currency(cartProducts[i]?.price,'IRR') }}</p>
              <p class="" :style="[`color:${ cartProducts[i]?.color};`]" > {{ cartProducts[i]?.color }}</p>
            </div>
            <!-- cart content end -->
            <!-- cart quantity -->
            <div class="flex border border-gray-300 text-gray-600 divide-x-reverse divide-gray-300">
              <div class="h-8 w-8 text-xl flex items-center justify-center cursor-pointer select-none" @click="removeCart(cartProducts[i]?.id)">-</div>
              <div class="h-8 w-10 flex items-center justify-center">{{ pro?.quantity }}</div>
              <div class="h-8 w-8 text-xl flex items-center justify-center cursor-pointer select-none" @click="addCart(cartProducts[i]?.id)">+</div>
            </div>
            <!-- cart quantity end -->
            <div class="ml-auto md:ml-0">
              <p class="text-primary text-lg font-semibold">{{currency(pro?.quantity *  cartProducts[i]?.price,'IRR') }}</p>
            </div>
            <div class="text-gray-600 hover:text-primary cursor-pointer">
              <i class="fas fa-trash"></i>
            </div>
          </div>
          <!-- single cart end -->
        </div>
        <!-- shipping carts end -->
      </div>
      <!-- product cart end -->
      <!-- order summary -->
      <div class="xl:col-span-3 lg:col-span-4 border border-gray-200 px-4 py-4 rounded mt-6 lg:mt-0">
        <h4 class="text-gray-800 text-lg mb-4 font-medium uppercase">جمع بندی سفارش</h4>
        <div class="space-y-1 text-gray-600 pb-3 border-b border-gray-200">
          <div class="flex justify-between font-medium">
            <p>تعداد انواع محصول</p>
            <p>{{ cart?.items_count }}</p>
          </div>
<!--          <div class="flex justify-between">-->
<!--            <p>Delivery</p>-->
<!--            <p>Free</p>-->
<!--          </div>-->
          <div class="flex justify-between">
            <p>Tax</p>
            <p>Free</p>
          </div>
        </div>
        <div class="flex justify-between my-3 text-gray-800 font-semibold uppercase">
          <h4>Total</h4>
          <h4>{{ currency(total) }}</h4>
        </div>

<!--        &lt;!&ndash; searchbar &ndash;&gt;-->
<!--        <div class="flex flex-row-reverse mb-5">-->
<!--          <input type="text" class="pl-4 w-full border border-r-0 border-primary py-2 px-3 rounded-l-md focus:ring-primary focus:border-primary text-sm" placeholder="Coupon">-->
<!--          <button type="submit" class="bg-primary border border-primary text-white px-5 font-medium rounded-r-md hover:bg-transparent hover:text-primary transition text-sm font-roboto">-->
<!--            Apply-->
<!--          </button>-->
<!--        </div>-->
<!--        &lt;!&ndash; searchbar end &ndash;&gt;-->

        <!-- checkout -->
        <a href="checkout.html" class="bg-primary border border-primary text-white px-4 py-3 font-medium rounded-md uppercase hover:bg-transparent
             hover:text-primary transition text-sm w-full block text-center">
          خرید
        </a>
        <!-- checkout end -->
      </div>
      <!-- order summary end -->
    </div>
    <!-- account content end -->
  <!-- account wrapper end -->

</template>

<style scoped>

</style>
