          ...
          <ul class="flex flex-col  p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white ">
            <li v-for="(category, i) in categories" :key="i" class="flex flex-row gap-1">
              ...
              <div v-if="currentCategory===category.id  && toggleMenu" @blur="toggleMegaMenu" id="mega-menu-icons-dropdown"
                   class=" flex rtl absolute mx-[20vw] w-[40vw] left-[10vw] translate-y-9 z-10 text-sm  rounded-lg border shadow-[#333] shadow-md bg-[#fefefe]">
                <div class="flex flex-col gap-[2px] rounded-l-lg bg-gray-50">
                  <router-link :to="`/category/${subCategories?.id}`"
                               class="flex p-2 pt-4  items-center text-[#666]  hover:text-blue-600 dark:hover:text-blue-500 group">
                    {{ subCategories?.title }}
                  </router-link>
                  <div v-for="(subCategory, j) in subCategories.children_recursive" :key="j"
                       class="p-2 pb-0 text-gray-900 md:pb-4 dark:text-[#333]">
                    <button @mouseover="setSub(subCategory)"
                            :class="[currentSubCategory===subCategory.id?'text-blue-600':'text-[#666]']"
                ...
                <div class="flex">
                  <ul class="flex flex-wrap gap-4 p-4" v-if="subC.children_recursive">
                    <li>
                      <router-link :to="`/category/${subC?.id}`"
                                   class="flex items-center text-[#666] dark:text-[#666] hover:text-blue-600 dark:hover:text-blue-500 group">
                        <span class="sr-only">{{ subC.title }}</span>
                        <svg
                            class="mr-2 w-4 h-4 text-[#666] dark:text-gray-500 group-hover:text-blue-600 dark:group-hover:text-blue-500"
                    ...
                    <li v-for="(sub,k) in subC?.children_recursive" :key="k">
                      <router-link :to="`/category/${sub?.id}`"
                                   class="flex items-center text-[#666] hover:text-blue-600 dark:hover:text-blue-500 group">
